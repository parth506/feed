            ...
            <div className="space-y-4 p-1">
              <div className="grid grid-cols-2 gap-2 text-center text-xs">
                <div className="p-3 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200/50">
                  ...
                  <p className="text-lg font-black text-emerald-600 dark:text-emerald-400">{evaluation.confusionMatrix.tp}</p>
                </div>
                <div className="p-3 rounded-lg bg-rose-50 dark:bg-rose-500/10 border border-rose-200/50">
                  ...
                  <p className="text-lg font-black text-rose-600 dark:text-rose-400">{evaluation.confusionMatrix.fp}</p>
                </div>
                <div className="p-3 rounded-lg bg-rose-50 dark:bg-rose-500/10 border border-rose-200/50">
                  <p className="text-[10px] text-slate-500">False Negative (FN)</p>
                  <p className="text-lg font-black text-rose-600 dark:text-rose-400">{evaluation.confusionMatrix.fn}</p>
                </div>
                <div className="p-3 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200/50">
                  <p className="text-[10px] text-slate-500">True Negative (TN)</p>
                  <p className="text-lg font-black text-emerald-600 dark:text-emerald-400">{evaluation.confusionMatrix.tn}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-[11px] pt-2 border-t border-slate-100 dark:border-slate-800">
                <div>
                  ...
                  <span className="font-bold text-slate-900 dark:text-slate-100">{(evaluation.precision * 100).toFixed(1)}%</span>
                </div>
                <div>
                  <span className="text-slate-400 block">Recall</span>
                  <span className="font-bold text-slate-900 dark:text-slate-100">{(evaluation.recall * 100).toFixed(1)}%</span>
                </div>
                <div>
                  <span className="text-slate-400 block">ROC-AUC</span>
                  <span className="font-bold text-brand-600 dark:text-brand-400">{evaluation.rocAuc.toFixed(3)}</span>
                </div>
              </div>
            </div>
          </ChartContainer>
